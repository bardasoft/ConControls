...
namespace ConControlsAnalyzer.Test.Verifiers
{
    ...
    public abstract partial class DiagnosticVerifier
    {
        ...
        private static void VerifyDiagnosticResults(IEnumerable<Diagnostic> actualResults, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expectedResults)
        {
            ...
            var compilerDiagnostics = actualResults as Diagnostic[] ?? actualResults.ToArray();
            int actualCount = compilerDiagnostics.Length;

            if (expectedCount != actualCount)
            {
                string diagnosticsOutput = compilerDiagnostics.Any() ? FormatDiagnostics(analyzer, compilerDiagnostics.ToArray()) : "    NONE.";
            ...
            {
                var actual = compilerDiagnostics.ElementAt(i);
        ...
        private static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, DiagnosticResultLocation expected)
        {
            ...
            Assert.IsTrue(actualSpan.Path == expected.Path || actualSpan.Path?.Contains("Test0.") == true && expected.Path.Contains("Test."),
